...
<div class="forabg">
	<div class="inner"><span class="corners-top"><span></span></span>
		...
		<div style="padding: 5px 5px 2px 5px; font-size: 1.1em; background-color: #ECF1F3; margin: 0px auto; text-align: center;">
			<?php echo (isset($this->_rootref['ADS_3'])) ? $this->_rootref['ADS_3'] : ''; ?>
...
<?php } $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; if ($_postrow_val['S_FIRST_UNREAD']) {  ?><a id="unread"></a><?php } ?>
	...
	<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post <?php if (($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST']) {  ?> online<?php } ?>">
		...
		<div class="postbody">
			...
			<?php } if ($_postrow_val['WARNING']) {  ?><div class="notice"><strong><?php if ($_postrow_val['WARNING_TYPE'] == (1)) {  echo ((isset($this->_rootref['L_BAN'])) ? $this->_rootref['L_BAN'] : ((isset($user->lang['BAN'])) ? $user->lang['BAN'] : '{ BAN }')); } else { echo ((isset($this->_rootref['L_WARNING'])) ? $this->_rootref['L_WARNING'] : ((isset($user->lang['WARNING'])) ? $user->lang['WARNING'] : '{ WARNING }')); } ?>:</strong> <?php echo $_postrow_val['WARNING_POSTER']; ?> <?php echo $_postrow_val['WARNING_TIME']; ?><br /><strong><?php echo ((isset($this->_rootref['L_REASON'])) ? $this->_rootref['L_REASON'] : ((isset($user->lang['REASON'])) ? $user->lang['REASON'] : '{ REASON }')); ?>:</strong> <em><?php echo $_postrow_val['WARNING']; ?></em></div><?php } if ($_postrow_val['BUMPED_MESSAGE']) {  ?><div class="notice"><br /><br /><?php echo $_postrow_val['BUMPED_MESSAGE']; ?></div><?php } ?>

			<div id="sig<?php echo $_postrow_val['POST_ID']; ?>" class="signature">
				<div style="float:right; display:block;"><?php echo $_postrow_val['SHAREON_LINK']; ?></div>
		...
		<?php if (! $_postrow_val['S_IGNORE_POST']) {  ?>
			...
			<dt>
				...
				<?php } ?><!-- We use "\r\n" newline here because insert_text() JS function doesn't insert newline with "\n" (at least in Windows) --><?php if ($this->_rootref['QUICK_REPLY']) {  ?><a href="#postform" onclick="insert_text('[b]<?php echo $_postrow_val['POSTER_QUOTE']; ?>[/b]\r\n'); return false;"<?php if ($_postrow_val['POST_AUTHOR_COLOUR']) {  ?> style="color: <?php echo $_postrow_val['POST_AUTHOR_COLOUR']; ?>" <?php } ?>><?php echo $_postrow_val['POST_AUTHOR']; ?></a><?php } else { if (! $_postrow_val['U_POST_AUTHOR']) {  ?><strong><?php echo $_postrow_val['POST_AUTHOR_FULL']; ?></strong><?php } else { echo $_postrow_val['POST_AUTHOR_FULL']; } } ?>

			</dt>

			<!-- We use "\r\n" newline here because insert_text() JS function doesn't insert newline with "\n" (at least in Windows) --><?php if (! $_postrow_val['RANK_TITLE']) {  ?><br /><?php if ($this->_rootref['QUICK_REPLY']) {  ?><dd><a href="#postform" onclick="addquote(<?php echo $_postrow_val['POST_ID']; ?>,'<?php echo $_postrow_val['POSTER_QUOTE']; ?>'); insert_text('\r\n'); return false;" title="<?php echo ((isset($this->_rootref['L_QUOTE_TEXT'])) ? $this->_rootref['L_QUOTE_TEXT'] : ((isset($user->lang['QUOTE_TEXT'])) ? $user->lang['QUOTE_TEXT'] : '{ QUOTE_TEXT }')); ?>"><?php echo ((isset($this->_rootref['L_QUOTE'])) ? $this->_rootref['L_QUOTE'] : ((isset($user->lang['QUOTE'])) ? $user->lang['QUOTE'] : '{ QUOTE }')); ?></a></dd><?php } } if ($_postrow_val['RANK_TITLE'] || $_postrow_val['RANK_IMG']) {  ?><dd><?php echo $_postrow_val['RANK_TITLE']; if ($this->_rootref['QUICK_REPLY']) {  ?><br /><a href="#postform" onclick="addquote(<?php echo $_postrow_val['POST_ID']; ?>,'<?php echo $_postrow_val['POSTER_QUOTE']; ?>'); insert_text('\r\n'); return false;" title="<?php echo ((isset($this->_rootref['L_QUOTE_TEXT'])) ? $this->_rootref['L_QUOTE_TEXT'] : ((isset($user->lang['QUOTE_TEXT'])) ? $user->lang['QUOTE_TEXT'] : '{ QUOTE_TEXT }')); ?>"><?php echo ((isset($this->_rootref['L_QUOTE'])) ? $this->_rootref['L_QUOTE'] : ((isset($user->lang['QUOTE'])) ? $user->lang['QUOTE'] : '{ QUOTE }')); ?></a><?php } if ($_postrow_val['RANK_TITLE'] && $_postrow_val['RANK_IMG']) {  ?><br /><?php } echo $_postrow_val['RANK_IMG']; ?></dd><?php } ?>


		<dd>&nbsp;</dd>

		<?php if ($_postrow_val['POSTER_POSTS'] != ('')) {  ?><dd><strong><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?>:</strong> <?php echo $_postrow_val['POSTER_POSTS']; ?></dd><?php } if ($_postrow_val['POSTER_JOINED']) {  ?><dd><strong><?php echo ((isset($this->_rootref['L_JOINED'])) ? $this->_rootref['L_JOINED'] : ((isset($user->lang['JOINED'])) ? $user->lang['JOINED'] : '{ JOINED }')); ?>:</strong> <?php echo $_postrow_val['POSTER_JOINED']; ?></dd><?php } if ($_postrow_val['POSTER_FROM']) {  ?><dd><strong><?php echo ((isset($this->_rootref['L_LOCATION'])) ? $this->_rootref['L_LOCATION'] : ((isset($user->lang['LOCATION'])) ? $user->lang['LOCATION'] : '{ LOCATION }')); ?>:</strong> <?php echo $_postrow_val['POSTER_FROM']; ?></dd><?php } if ($_postrow_val['POSTER_BANNED']) {  ?><br /><dd style="background-color: #ECD5D8;"><strong><?php echo ((isset($this->_rootref['L_BANNED'])) ? $this->_rootref['L_BANNED'] : ((isset($user->lang['BANNED'])) ? $user->lang['BANNED'] : '{ BANNED }')); ?>:</strong> <?php echo $_postrow_val['POSTER_BAN_END']; } else if ($_postrow_val['POSTER_WARNINGS']) {  ?><dd style="background-color: #FFFF99;"><strong><?php echo ((isset($this->_rootref['L_WARNINGS'])) ? $this->_rootref['L_WARNINGS'] : ((isset($user->lang['WARNINGS'])) ? $user->lang['WARNINGS'] : '{ WARNINGS }')); ?>:</strong> <?php echo $_postrow_val['POSTER_WARNINGS']; ?></dd><?php } if ($_postrow_val['S_PROFILE_FIELD1']) {  ?><!-- Use a construct like this to include admin defined profile fields. Replace FIELD1 with the name of your field. -->
	...
	<?php if (( $this->_rootref['ADS_5'] && $_postrow_val['S_FIRST_ROW'] ) || ( $this->_rootref['ADS_6'] && ! $_postrow_val['S_FIRST_ROW'] ) || ( $this->_rootref['ADS_9'] && $_postrow_val['S_ROW_COUNT'] == (2) )) {  ?>

	<div class="post bg3">
		<div class="inner"><span class="corners-top"><span></span></span>
			<div class="postbody" style="width: 100%;">
				<?php if ($this->_rootref['ADS_9'] && $_postrow_val['S_ROW_COUNT'] == (2) && ! $this->_rootref['S_USER_LOGGED_IN'] && $this->_rootref['S_REGISTER_ENABLED'] && ! ( $this->_rootref['S_SHOW_COPPA'] || $this->_rootref['S_REGISTRATION'] )) {  ?>

					<div style="margin-bottom: 3px; text-align: center;">
						<a href="<?php echo (isset($this->_rootref['U_REGISTER'])) ? $this->_rootref['U_REGISTER'] : ''; ?>">Для отключения просмотра рекламы зарегистрируйтесь на форуме.</a>
					</div>
				<?php } ?>

				<div style="font-size: 1.1em; margin: 0px auto; text-align: center;">
					<?php if ($_postrow_val['S_FIRST_ROW']) {  echo (isset($this->_rootref['ADS_5'])) ? $this->_rootref['ADS_5'] : ''; } else if ($this->_rootref['ADS_9'] && $_postrow_val['S_ROW_COUNT'] == (2)) {  echo (isset($this->_rootref['ADS_9'])) ? $this->_rootref['ADS_9'] : ''; } else { echo (isset($this->_rootref['ADS_6'])) ? $this->_rootref['ADS_6'] : ''; } ?>
...
<?php if ($this->_rootref['QUICK_REPLY']) {  $this->_tpl_include('quick_reply.html'); } if ($this->_rootref['ADS_4']) {  ?>
